...
const StockModal = ({ active, handleModal, id, setError, getTotalDividends}) => {
    ...
    useEffect(() => {
        const getStockById = async () => {
            try {
                const response = await axios.get(`http://localhost:8000/individual_stock_dividends/${id}`);
      ...
      const handleCreateStock = async (e) => {
        e.preventDefault();
        const url = `http://localhost:8000/individual_stock_dividends`;
        const data = {
            symbol: symbol,
            shares: shares,
      ...
      const handleUpdateStock = async (e) => {
        e.preventDefault();
        const url = `http://localhost:8000/individual_stock_dividends/${id}`;
        const data = {
            symbol: symbol,
            shares: shares,
            purchase_date: purchaseDate,
